    ...
    initialize() {
      ...
      _.each(toCache, (cacheConf) => {
        ...
        router.get(endpoint, async (ctx, next) => {
          if (ctx.request.query.hasOwnProperty('__bypassCache__')) {
            if (ctx.request.query['__bypassCache__'] === 'true') {
              // deleting the queryParam otherwise strapi will try to filter model on it
              delete ctx.request.query['__bypassCache__']
              info('Bypassing cache due to query param')
              await next();
              return
            }
          }
